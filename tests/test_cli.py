...
def test_delta_nbr_run(tmp_path):
    ...
    code = cli([
        ...
        "--pre-swir2", pre_swir2,
        "--post-nir", post_nir,
        "--post-swir2", post_swir2,
        "--out", str(out_path),
    ])
    assert code == 0
    assert out_path.exists()
    arr = load_npy(out_path)
    pre_nbr = (load_npy(pre_nir) - load_npy(pre_swir2)) / (load_npy(pre_nir) + load_npy(pre_swir2))
    post_nbr = (load_npy(post_nir) - load_npy(post_swir2)) / (load_npy(post_nir) + load_npy(post_swir2))
...
def test_png_preview_invalid_with_multi_indices(tmp_path):
    ...
    with pytest.raises(SystemExit):
        cli([
            ...
            "--red", red,
            "--out-dir", str(out_dir),
            "--png-preview", str(out_dir / "preview.png"),
        ])
...
def test_mask_application(tmp_path):
    ...
    assert code == 0
    out = load_npy(out_path)
    assert np.isnan(out[1])  # masked pixel becomes NaN after computation (inputs were NaN)
